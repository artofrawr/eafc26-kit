...
/**
 * Solver for the "Daily Silver Upgrade" SBC
 *
 * This is one of the simplest SBCs that requires:
 * - Using the squad builder
 * - Configuring specific toggles and sort options
 * - Building the squad automatically with Silver quality filter
 */
export class DailySilverUpgradeSolver {
  ...
  /**
   * Solve the Daily Silver Upgrade SBC
   * NOTE: This assumes you are already on the SBC favourites page
   */
  async solve(): Promise<void> {
    console.log('Starting Daily Silver Upgrade solver...');

    try {
      // Step 1: Click on the SBC tile
      await this.helpers.clickSBCTile('Daily Silver Upgrade');
      ...
      // Step 6: Select "Silver" from Quality filter
      ...
      console.log('Daily Silver Upgrade solver completed successfully!');
    } catch (error) {
      console.error('Daily Silver Upgrade solver failed:', error);
      throw error;
    }
  }
}
