...
/**
 * Solver for the "Daily Rare Gold Upgrade" SBC
 *
 ...
 */
export class DailyRareGoldUpgradeSolver {
  ...
  /**
   * Solve the Daily Rare Gold Upgrade SBC
   * NOTE: This assumes you are already on the SBC favourites page
   */
  async solve(): Promise<void> {
    console.log('Starting Daily Rare Gold Upgrade solver...');

    try {
      // Step 1: Click on the SBC tile
      await this.helpers.clickSBCTile('Daily Rare Gold Upgrade');
      ...
      // Step 6: Select "Gold" from Quality filter
      await this.helpers.selectQualityFilter('Gold');
      ...
      console.log('Daily Rare Gold Upgrade solver completed successfully!');
    } catch (error) {
      console.error('Daily Rare Gold Upgrade solver failed:', error);
      throw error;
    }
  }
}
